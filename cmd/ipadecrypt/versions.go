...
func versionsHandler(cmd *cobra.Command, args []string) {
	...
	app, err := lookupVersionsTargetApp(as, acc, target)
	if err != nil {
...
func showVersionsWarning() error {
	...
	fmt.Fprintln(w)
	fmt.Fprintln(w, "  Each version opened hits Apple's private endpoint. Too many")
	fmt.Fprintln(w, "  in a short window can get your Apple ID \x1b[33mflagged, rate-limited,")
	fmt.Fprintln(w, "  or permanently banned\x1b[0m. Only the 3 newest versions load eagerly;")
	fmt.Fprintln(w, "  older ones fetch on Enter. Results cached on disk across runs.")
	fmt.Fprintln(w)
