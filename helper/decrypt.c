...
int decrypt_appexes(const char *bundle_src, const char *bundle_dst) {
    ...
    for (int si = 0; subdirs[si]; si++) {
        ...
        if (!d) { if (errno != ENOENT) { int saved = errno; attrs_t a; attrs_init(&a); attrs_str(&a, "path", dir_src); attrs_errno(&a, saved); emit(LOG_WARN, "appex.scan.open_failed", &a, NULL); } continue; }
        struct dirent *e;
        while ((e = readdir(d))) {
...
static int zwrite(int fd, const void *buf, size_t n) {
    ...
    for (size_t off = 0; off < n; ) {
        ...
        if (w < 0) { if (errno == EINTR) continue; LOG_ERRNO("zip.write_failed", "archive write to fd %d failed: %s", fd, strerror(errno)); return -1; }
        if (w == 0) { errno = EIO; LOG_ERRNO("zip.write_stalled", "archive write to fd %d made no progress", fd); return -1; }
        off += (size_t)w;
    }
    return 0;
}
...
static int zip_add_file(int out, const char *full, const char *rel,
    ...
    if (fd < 0) {
        ...
        int saved = errno; attrs_t a; attrs_init(&a); attrs_str(&a, "path", rel); attrs_errno(&a, saved); emit(LOG_WARN, "zip.file.open_failed", &a, "skipping unreadable %s: %s", rel, strerror(saved));
        return 1;
    }
    struct stat st;
    if (fstat(fd, &st) != 0) {
        int saved = errno; attrs_t a; attrs_init(&a); attrs_str(&a, "path", rel); attrs_errno(&a, saved); emit(LOG_WARN, "zip.file.stat_failed", &a, "skipping %s: fstat: %s", rel, strerror(saved));
        close(fd);
        return 1;
    }
    ...
    *cursor += 30 + namelen;
    ...
    int zrc = deflateInit2(&zs, 1, Z_DEFLATED, -15, 8, Z_DEFAULT_STRATEGY);
    if (zrc != Z_OK) {
        attrs_t a; attrs_init(&a); attrs_str(&a, "path", rel); attrs_int(&a, "zlib", zrc); emit(LOG_ERROR, "zip.deflate_init_failed", &a, NULL);
        close(fd);
        return -1;
    }
    ...
    do {
        ...
        do {
            ...
            int drc = deflate(&zs, flush);
            if (drc != Z_OK && drc != Z_STREAM_END) { attrs_t a; attrs_init(&a); attrs_str(&a, "path", rel); attrs_int(&a, "zlib", drc); emit(LOG_ERROR, "zip.deflate_failed", &a, NULL); rc = -1; break; }
    ...
    *cursor += 16;
    ...
    if (!e) { emit(LOG_ERROR, "zip.entry.oom", NULL, "could not allocate archive entry"); return -1; }
    e->name = strdup(rel);
    if (!e->name) { emit(LOG_ERROR, "zip.name.oom", NULL, "could not allocate archive entry name"); free(e); return -1; }
    e->crc = crc;
    ...
    *tail = e;
    if (log_level_visible(LOG_DEBUG)) { attrs_t a; attrs_init(&a); attrs_str(&a, "path", rel); attrs_uint(&a, "size", usize); attrs_uint(&a, "compressed", csize); attrs_hex(&a, "crc32", crc); emit(LOG_DEBUG, "zip.file.done", &a, NULL); }
    return 0;
}
...
static int zip_walk(int out, const char *base, const char *rel,
    ...
    DIR *d = opendir(dir);
    if (!d) { LOG_ERRNO("zip.walk.open_failed", "opendir %s failed: %s", dir, strerror(errno)); return 0; }
    struct dirent *de;
    int rc = 0;
    while (rc == 0 && (de = readdir(d))) {
        ...
        if (zip_excluded(child)) { if (log_level_visible(LOG_DEBUG)) { attrs_t a; attrs_init(&a); attrs_str(&a, "path", child); emit(LOG_DEBUG, "zip.file.excluded", &a, NULL); } continue; }
        snprintf(full, sizeof(full), "%s/%s", base, child);
        struct stat st;
        if (lstat(full, &st) != 0) { LOG_ERRNO("zip.walk.stat_failed", "lstat %s failed: %s", full, strerror(errno)); continue; }
        if (S_ISLNK(st.st_mode) && stat(full, &st) != 0) { LOG_ERRNO("zip.walk.symlink_failed", "stat symlink %s failed: %s", full, strerror(errno)); continue; }  // deref
...
int run_zip(const char *staging, const char *ipa_path) {
    attrs_t begin; attrs_init(&begin); attrs_str(&begin, "staging", staging); attrs_str(&begin, "output", ipa_path); emit(LOG_DEBUG, "zip.begin", &begin, NULL);
    int stream_stdout = ipa_path && ipa_path[0] == '-' && ipa_path[1] == '\0';
    int out = stream_stdout
                  ? STDOUT_FILENO
                  : open(ipa_path, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (out < 0) {
        LOG_ERRNO("zip.output.open_failed", "open %s failed: %s", ipa_path, strerror(errno));
        return -1;
    }
    ...
    if (rc == 0 && !head) {
        attrs_t a; attrs_init(&a); attrs_str(&a, "staging", staging); emit(LOG_ERROR, "zip.empty", &a, "staging has no Payload contents");
        rc = -1;
    }
    ...
    if (!stream_stdout && close(out) != 0) { LOG_ERRNO("zip.output.close_failed", "close %s failed: %s", ipa_path, strerror(errno)); rc = -1; }
    attrs_int(&begin, "result", rc); attrs_uint(&begin, "entries", count); attrs_uint(&begin, "bytes", cursor + cd_size + (rc == 0 ? 22 : 0));
    emit(rc == 0 ? LOG_DEBUG : LOG_ERROR, "zip.done", &begin, rc == 0 ? NULL : "zip writer failed");
    return rc;
}
