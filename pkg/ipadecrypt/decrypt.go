...
func Decrypt(ctx context.Context, req Request) (_ *Result, returnErr error) {
	...
	patch, err := patchSource(encryptedPath, probe.IOSVersion, probe.DeviceFamily, req.PatchDeviceType)
	if err != nil {
	...
	plan, err := buildInstallPlan(dev, patch.uploadPath, bundleID)
	if err != nil {
		return nil, err
	}
	if !req.KeepRemoteFiles {
		cleanups.push(func() error { _ = dev.Remove(plan.stagingRemote); return nil })
	}
...
func patchSource(source, iosVersion string, deviceFamily int, patchDeviceType bool) (patchResult, error) {
	file, err := os.CreateTemp("", "ipadecrypt-patched-*.ipa")
	if err != nil {
...
func uploadAndInstall(dev *device.Client, plan installPlan, uploadPath string, reinstalled bool, notify func(string, string), progress func(int64, int64)) (installResult, error) {
	...
	if err := dev.Upload(reader, plan.stagingRemote, 0); err != nil {
		return installResult{}, err
	}
